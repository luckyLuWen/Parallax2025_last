...
static void lcrp_jacobianEachPts(double *R, double* KR, double* KdA, double *KdB, double* KdG, double* pa, double* ppt, int nM, int nN, int nP,double* pAM, double* pAA, double* pPA, double* pPB)
{
	...
	matDlcDXc[0] = matDlcDxyt[0] * matDxytDXc[0] + matDlcDxyt[1] * matDxytDXc[1] + matDlcDxyt[2] * matDxytDXc[2];//lc关于Xc的一阶导
	matDlcDYc[0] = matDlcDxyt[0] * matDxytDYc[0] + matDlcDxyt[1] * matDxytDYc[1] + matDlcDxyt[2] * matDxytDYc[2];//lc关于Yc的一阶导
	matDlcDZc[0] = matDlcDxyt[0] * matDxytDZc[0] + matDlcDxyt[1] * matDxytDZc[1] + matDlcDxyt[2] * matDxytDZc[2];//lc关于Zc的一阶导
	...
	matDlcDRB[0] = matDlcDxyt[0] * matDxytDRB[0] + matDlcDxyt[1] * matDxytDRB[1] + matDlcDxyt[2] * matDxytDRB[2];//lc关于phi的一阶导
	matDlcDRG[0] = matDlcDxyt[0] * matDxytDRG[0] + matDlcDxyt[1] * matDxytDRG[1] + matDlcDxyt[2] * matDxytDRG[2];//lc关于omega的一阶导

	pPA[0] = matDlcDRB[0];			pPA[1] = matDlcDRG[0];//lc对外参的一阶导
	pPA[2] = matDlcDXc[0];			pPA[3] = matDlcDYc[0];			pPA[4] = matDlcDZc[0];

	pPB[0] = matDlcDX[0];		pPB[1] = matDlcDY[0];		pPB[2] = matDlcDZ[0];//lc对三维点的一阶导
	...
	double Tau )
{
	...
	lcrp_initialize( m_szCameraInit, m_szFeatures, m_szCalibration, m_szXYZ );     //给定初值
...
bool CLCRP::lcrp_initialize( char* szCamera, char* szFeature,  char* szCalib, char* szXYZ )
{
	...
	{
		lcrp_saveInitialXYZ("C:/txtdata/Village/xyz_Maxdw.txt", lcrp_angle2xyz(m_motstruct));//转换为xyz
	}
	else//Add by Lu
		;//Other codes
...
{	
	...
	nuis = lcrp_ConstructSmask( Sidxij, Uidxij );   //返回U矩阵非零元素个数
	...
	S	=	(double *)malloc(m_nS*Sblsz*sizeof(double));          //S矩阵：非0元素个数*5*5
	W	=	(double *)malloc(nvis*Wsz*sizeof(double));            //W矩阵：像点数*5*3
	U	=	(double *)malloc(nuis*Usz*sizeof(double));            //U矩阵：非0元素个数×5*5
	...
	if ( ft == LCRP_FixDefault )
	{
		...
		tmp = abs(*(m_motstruct+7)-*(m_motstruct+2));//第2个相机的Xc-第1个相机的Xc
		ft = LCRP_FixX;									
		tmp1 = abs(*(m_motstruct+8)-*(m_motstruct+3));//第2个相机的Yc-第1个相机的Yc
		if ( tmp1 > tmp )
		{ft= LCRP_FixY; tmp = tmp1;	}
		tmp1 = abs(*(m_motstruct+9)-*(m_motstruct+4));//第2个相机的Zc-第1个相机的Zc
		if ( tmp1 > tmp )
		{ft= LCRP_FixZ; }
	}
	nft = ft;
	...
	lcrp_constructAuxCSSGN( Ap, Aii );//构造稀疏矩阵的索引结构，从1行1列开始，存储每列的起始索引,存储非零元素的行索引
	...
	m_cholSparseE = cholmod_zeros( cnp*m-6, 1, CHOLMOD_REAL, &m_cS);//固定第1帧的外参和第2帧的Xc轴，实际未知参数个数是5*m-6
	Ex = (double*)m_cholSparseE->x;
	
	//(m_nS-m_ncams)表示S矩阵中非对角块（相机之间的约束）的数量，其中的每个非对角块占据5*5个非零元素（不对称）
	//m_ncams表示S矩阵中对角块（单个相机自身的约束）的数量，其中每个对角块占据15个非零元素（对称，只存储下三角部分）
	...
	{
		//Setup S matrix, include two step
		memset( U, 0, nuis*Usz*sizeof(double) );//nuis是U矩阵的非零元素个数，Usz=5*5，存储U矩阵的非零元素
		memset( ea, 0,m*easz*sizeof(double) );//m是相机数量，easz=5，ea存储相机参数
		memset( V, 0, n*Vsz*sizeof(double));//n是三维点数量，Vsz=3*3，
		memset( eb, 0, n*ebsz*sizeof(double));//n是三维点数量，ebsz=3，eb存储三维点参数
		memset( W, 0, nvis*Wsz*sizeof(double));	//nvis是像点数，Wsz=5*3
		...
		{
			for ( ii = 0; ii < 2+nft; ii++ )
				dpa[5+ii] = rx[ii];//存储第2帧的2个姿态
			for ( ii = 0; ii < 5*m-(8+nft); ii++ )
				dpa[ii+8+nft] = rx[2+nft+ii];//跳过第2帧的Xc，存储解算的相机参数
		}			
		//for (int i = 0; i < m; i++)
...
int CLCRP::lcrp_ConstructSmask( sba_crsm& Sidxij, sba_crsm& Uidxij )//分别是S矩阵和U矩阵的稀疏索引
{
	int i, j, k, ii, jj;
		...
		for ( ii = 1; ii < m; ii++ )  //column，第0个视图是参考帧，通常被固定，不作为优化变量
		{
			for ( k = 0; k < 5; k++ )
			{
				*Sp = nZ;//表示稀疏矩阵中每一列的起始位置（非零元素的索引）,0,1,3,6,
				//printf("%d ", *Sp);
				if ((ii*5+k)==(7+nft))//k=2代表Xc，3代表Yc，4代表Zc。nft=0代表固定Xc，1代表固定Yc，2代表固定Zc
				...
				for ( jj = 1; jj <= ii; jj++ )	//row
				{
					if ((m_smask[jj*m+ii]==1))
					{
						pos1 = sba_crsm_elmidx( &Sidxij, jj, ii );
						//printf("%d ", pos1);
						ptr5 = S + pos1*25;//指向S矩阵对应（jj,ii）的子矩阵数据
						
						if( ii == jj )//对角线块
						{
							for ( jjj = 0; jjj <= k; jjj++)
							{
								if ( (jj*5+jjj) != (7+nft))//除了jj=1，jjj=3（代表第2个相机的Xc固定）外，
								{
									if (jj * 5 + jjj < 7 + nft)//第2个相机的姿态角
										*Si++ = jj * 5 + jjj - 5;
									else//第2个相机的Yc，Zc
										*Si++ = jj * 5 + jjj - 6;
	...
	int** nfeature, int** archorSort)
{
	...
	*motstruct	=	(double *)malloc( (*ncams*5 + *n3Dpts*3)*sizeof(double) );//用于存储所有的外方位元素 和 所有特征点的3D坐标
	if(	*motstruct==NULL )
	{
	...
	*archorSort = (int*)malloc(*n3Dpts*3*sizeof(int));
	...
	if (zu == 1)
	{
		...
		lcrp_updateKR(m_R, m_KR, m_KdA, m_KdB, m_KdG, m_K, *motstruct);//第5个参数为相机内参，第6个参数为相机外参
...
void CLCRP::lcrp_jacobian(double *p, int* archor,struct sba_crsm *Uidxij, double*e,double* U,double* ea, double* V, double* eb, double *W, int n, int m, int numprojs, int *nphoto, int *nfeature )
{
	...
	for ( i = 0; i < m_n3Dpts; i++ )
	{
		...
		for ( j = 0; j < numfea; j++ )
		{
			...
			//printf("%d  ", pos);
			ppUpa = U + pos*(5*5);//按块填充，会不会存在同一块被重复填充的问题？同一块是不同观测偏导的相加过程，不会被重复填充

			//printf("\n\n\n\n");
			for ( ii = 0; ii < 5; ii++ ) for( jj = ii; jj < 5; jj++ )	//由于是对称矩阵，只填上三角
			{
				for ( k = 0, sum = 0; k < 1; k++ )
...
void CLCRP::lcrp_cost(double* p, double* hx, int* archor)
{
	...
	for (i = 0; i < m_n2Dprojs; i++)
	{
		...
		ppt = pb + nF * pnp;//三维点坐标
		ppa = pa + nP * cnp + 2;//锚点位置指针
		pmeas = hx + i * mnp; // set pmeas to point to hx_ij 存储重投影光锥
