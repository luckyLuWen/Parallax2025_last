...
static void sba_reprojectEachPts( double *KR, double* pa, double* pb, int nM, int nN, int nP, double n[2] )
{
	double *posei, *posek;
	//double pti2k[3];
	//double ptXUnit[3];
	...
	//posei = pa + 6 * nP + 3;//np编号视图的Position
...
static void sba_jacobianEachPts(double* KR, double* KdA, double *KdB, double* KdG, double* pa, double* ppt, int nM, int nN, int nP,double* pAM, double* pAA, double* pPA, double* pPB)
{
	double matXj[3];
	double matxyt[3];
	double matDuvDxyt[6];
	...
	//printf("%f %f %f %f %f %f\n", ppt[0], ppt[1], ppt[2], posei[0], posei[1], posei[2]);
	matXj[0] = ppt[0] - ppa[0];
	matXj[1] = ppt[1] - ppa[1];
	matXj[2] = ppt[2] - ppa[2];
	pKR = KR + nP * 9;
...
bool CSBA::sba_run( bool bRobust,
	bool bLM,
	int nMaxIter,
	char* szCam,
	char* szFea,
	...
	double Tau )
{
	...
	sba_initialize( m_szCameraInit, m_szFeatures, m_szCalibration, m_szXYZ );     //给定初值
...
bool CSBA::sba_run( int argc, char** argv )
{
	bool bTrue = sba_parseArgs( argc, argv );
	if (!bTrue)
	{
	...
	sba_initialize( m_szCameraInit, m_szFeatures, m_szCalibration, m_szXYZ );
...
bool CSBA::sba_initialize( char* szCamera, char* szFeature,  char* szCalib, char* szXYZ )
{
	...
	if ( szCalib != NULL )
	{
		...
		sba_readCameraPoseration(szCalib, m_K);                            //
		///*-------------------------------------------------调试用---------------------------------------------------*/
		//for (int i = 0;i < 9;i++)
		//{
		//	printf("%f ", m_K[i]);
	...
	if(savePara == 1)//Add by Lu
		sba_saveInitialParallax("C:/txtdata/Village/InitialParallax.txt", m_motstruct);//Add by Lu
	//Modify the directory to export point file
	if (zu == 1)
		sba_saveTriangulatedxyz("C:/txtdata/Village/Triangulatedxyz.txt", m_motstruct);
	else if (zu == 2)//Add by Lu
	{
		sba_saveInitialXYZ("C:/txtdata/Village/xyz_Maxdw.txt", sba_angle2xyz(m_motstruct));//转换为xyz
	}
	else//Add by Lu
		;//Other codes
...
double *CSBA::sba_angle2xyz(double* p)
{
...
void CSBA::sba_saveInitialXYZ(char* sz3Dpt, double* p)
{
	static int i = 0;
	double dx, dy, dz;
...
void CSBA::sba_saveInitialParallax(char* sz3Dpt, double* p)
{
	//double* n3DptsT = (double*)malloc((m_n3Dpts * 3) * sizeof(double));
...
void CSBA::sba_saveTriangulatedxyz(char* sz3Dpt, double* p)
{
	static int i = 0;
	double x, y, z;
	FILE* fp = NULL;
...
bool CSBA::sba_motstr_levmar( )
{
	...
	nuis = sba_ConstructSmask( Sidxij, Uidxij );
	nobs=nvis*mnp;
	nvars=m*cnp + n*pnp;
	...
	for(itno=0; itno<m_nMaxIter && !stop; ++itno)
	{
		...
		if( itno == 0)//iter No，第一次就是0就肯定会执行
			mu = sba_computeInitialmu( U, V, Uidxij, tau, nvars );
		...
		{//雅各布矩阵表示重投误差相对于参数的导数。
			...
			t11 = clock();
			//求V的逆矩阵V^-1
			sba_inverseVLM( V, IV, Uidxij, mu ); //compute inverse matrix of V
			
			//Step two: construct S matrix using U V W, S = U - W*V^-1*W^T
			memset( E, 0, m*easz*sizeof(double));
			memset( S, 0, m_nS*Sblsz*sizeof(double) );
			sba_constructSLM( S, E, U, IV, W, ea, eb, Sidxij, mu );
			t2 = clock();
			
			//Solve linear equation
			//set CSS format using S matrix
			sba_constructCSSLM( Si, Sp, Sx, S, m_cholSparseS, Sidxij, init ); 
			for ( ii = 0; ii < cnp*m; ii++  )
				Ex[ii] = E[ii];	
			
			sba_solveCholmodLM( Ap, Aii, init, ordering);
			nLinear++;
			...
			{
				//Solve features
				sba_solveFeatures( W, IV, ea, eb, dpa, dpb );
				...
				sba_updateKR( m_KR, m_KdA, m_KdB, m_KdG, m_K, pdp );
				t4 = clock();
...
iterstop:
	...
	//sba_saveXYZ( m_szCamePose, m_sz3Dpts, p, false );
...
//#pragma optimize("", off) // 关闭优化
bool CSBA::sba_motstr_gn( FixType ft )
{	
	if ( m_motstruct == NULL || m_imgpts == NULL || m_K == NULL )
	{	
	...
	nuis = sba_ConstructSmask( Sidxij, Uidxij );   //返回U矩阵非零元素个数
	...
	sba_constructAuxCSSGN( Ap, Aii );//构造稀疏矩阵的索引结构，从1行1列开始，存储每列的起始索引,存储非零元素的行索引
	...
	for(itno=0; itno<m_nMaxIter && !stop; ++itno)
	{
		...
		memset( W, 0, nvis*Wsz*sizeof(double));	//nvis是像点数，Wsz=18
		...
		sba_inverseVGN( U, V, Uidxij ); //compute inverse matrix of V

		//Step two: construct S matrix using U V W, S = U - W*V^-1*W^T
		memset( E, 0, m*easz*sizeof(double));
		memset( S, 0, m_nS*Sblsz*sizeof(double) );
		sba_constructSGN( S, E, U, V, W, ea, eb, Sidxij );//S为方程左侧，E为方程右侧
		t2 = clock();
	
		//Solve equation
		sba_constructCSSGN( Si, Sp, Sx, S, m_cholSparseS, Sidxij, init, nft ); //set CSS format using S matrix//break
		...
		sba_solveCholmodGN( Ap, Aii, init, ordering);//break
		init = true;
		...
		//Solve features
		sba_solveFeatures( W, V, ea, eb, dpa, dpb );//break
		...
		sba_updateKR( m_KR, m_KdA, m_KdB, m_KdG, m_K, p );
		t4 = clock();
...
double CSBA::sba_computeInitialmu(double* U, double* V, sba_crsm& Uidxij, double tau, int nvars )
{
	int i, j;
...
void CSBA::sba_inverseVLM( double* V, double* IV, sba_crsm& Uidxij, double mu )
{
	int i, j;
...
void CSBA::sba_inverseVGN( double* U, double* V, sba_crsm& Uidxij )
{
	int i;
	int m = m_ncams, n = m_n3Dpts;
	int Usz = 36, Vsz = 9, pnp = 3, cnp = 6;
...
int CSBA::sba_ConstructSmask( sba_crsm& Sidxij, sba_crsm& Uidxij )//分别是S矩阵和U矩阵的稀疏索引
{
	int i, j, k, ii, jj;
...
void CSBA::sba_constructAuxCSSLM( int *Ap, int *Aii )
{
	int* Cp = Ap;
	int* Ci = Aii;
	int ii, jj;
...
void CSBA::sba_constructAuxCSSGN( int *Ap, int *Aii )
{
	//从0行0列开始是因为固定第一帧
	int* Cp = Ap;//存储每列的起始索引
...
void CSBA::sba_constructSLM( double* S, double* E, double* U, double* V, double* W, double* ea, double* eb, sba_crsm& Sidxij, double mu )
{
	int i, j, ii, jj, k, l;
...
void CSBA::sba_constructSGN( double* S, double* E, double* U, double* V, double* W, double* ea, double* eb, sba_crsm& Sidxij )
{
	int i, j, ii, jj, k, l;
	int m = m_ncams, cnp = 6, pnp = 3, Usz = 36, ebsz=3;
	int pos, pos1, numfea;
...
void CSBA::sba_solveFeatures( double *W, double *IV, double *ea, double *eb, double *dpa, double *dpb)
{
...
void CSBA::sba_constructCSSLM( int* Si, int* Sp, double* Sx, double* S, cholmod_sparse* m_cholSparseS, sba_crsm& Sidxij, bool init)
{
	int ii, jj, jjj, k;
	int pos1, m = m_ncams;
...
void CSBA::sba_constructCSSGN( int* Si, int* Sp, double* Sx, double* S, cholmod_sparse* m_cholSparseS, sba_crsm& Sidxij, bool init, int nft)
{
	int ii, jj, jjj, k;
	int pos1, m = m_ncams;
	//Copy S matrix and E matrix to specific format structure for Cholmod 
...
bool CSBA::sba_solveCholmodLM( int* Ap, int* Aii, bool init, bool ordering)
{
	int i, j;
...
bool CSBA::sba_solveCholmodGN( int* Ap, int* Aii, bool init, bool ordering)
{
	int i, j;
	int m = m_ncams;
	VectorXi scalarPermutation, blockPermutation;
...
int CSBA::countNDoubles(FILE *fp)
{
	int lineno, ch, np, i;
...
int CSBA::skipNDoubles(FILE *fp, int nvals)
{
	register int i;
	int j;
...
void CSBA::readNpointsAndNprojections(FILE *fp, int *n3Dpts, int pnp, int *nprojs, int mnp)
{
...
void CSBA::sba_readCablibration(FILE* fp, double *K )
{
...
void CSBA::sba_readCameraPose( FILE *fp, double *params )
{
	int n, num, lineno = 0 ;
	double *tofilter;
...
int CSBA::readNInts(FILE *fp, int *vals, int nvals)
{
	register int i;
	int n, j;
...
int CSBA::readNDoubles(FILE* fp, double* vals, int nvals)
{
	register int i;
	int n, j;
...
//#pragma optimize("", off) // 关闭优化
void CSBA::sba_readProjectionAndInitilizeFeature(FILE *fp,
...
void CSBA::sba_readProjectionAndTriangulateFeature(FILE* fp, double* projs, int ncams)
{
	int n;
	int nframes;
	int ptno = 0;
...
//#pragma optimize("", off) // 关闭优化
void CSBA::sba_readAndInitialize(char *camsfname, char *ptsfname, int *ncams,
	...
	int** nfeature, int** archorSort)
{
	...
	*archorSort = (int*)malloc(*n3Dpts*3*sizeof(int));
	...
	sba_readCameraPose(fpc, *motstruct);
	///*-------------------------------调试用-----------------------------------------*/
	...
	if (zu == 1)
	{
		...
		sba_constructP(m_P, m_K, *motstruct);
	}
	else
		sba_updateKR(m_KR, m_KdA, m_KdB, m_KdG, m_K, *motstruct);//第5个参数为相机内参，第6个参数为相机外参
	...
	//for (int i = 0; i < m_ncams; i++)
	//{
	//	printf("%d %f %f %f %f %f %f\n", i + 1, (*motstruct)[i * 6 + 0], (*motstruct)[i * 6 + 1], (*motstruct)[i * 6 + 2], (*motstruct)[i * 6 + 3], (*motstruct)[i * 6 + 4], (*motstruct)[i * 6 + 5]);
	//}
	//for (int i = 0; i < m_n3Dpts; i++)
	//{
	//	printf("%d %f %f %f\n", i + 1, (*motstruct)[m_ncams * 6 + i * 3 + 0], (*motstruct)[m_ncams * 6 + i * 3 + 1], (*motstruct)[m_ncams * 6 + i * 3 + 2]);
	//}
	...
	*/
	if(zu==1)
		sba_readProjectionAndTriangulateFeature(fpp, *imgpts, *ncams);
	else
	{
		//只用来设置umask和smask
	...
	//	{
			...
			//double dispti2k;
			//dispti2k = sqrt( pti2k[0]*pti2k[0] + pti2k[1]*pti2k[1]  + pti2k[2]*pti2k[2] );//模长
			...
			//double disDot2;
			//disDot2 = ptMain[0]*pti2k[0] + ptMain[1]*pti2k[1] + ptMain[2]*pti2k[2]; //点积
			//double dww = disDot2/(dispti2k * sqrt(dW1));//两向量夹角

			//double* pKR = m_KR + nM*9;//主锚点的KR矩阵
			...
			//
			//int id1 = cur + m_archorSort[i*2];//主锚点编号
			//int id2 = cur + m_archorSort[i*2+1];//副锚点编号
			//double err1 = (m_imgpts[id1*2]-n[0])*(m_imgpts[id1*2]-n[0])+(m_imgpts[id1*2+1]-n[1])*(m_imgpts[id1*2+1]-n[1]);//三维点在主锚点的重投影残差平方
			//double err2 = (m_imgpts[id2*2]-n2[0])*(m_imgpts[id2*2]-n2[0])+(m_imgpts[id2*2+1]-n2[1])*(m_imgpts[id2*2+1]-n2[1]);//三维点在副锚点的重投影残差平方

			//cur += m_archor[i*3];//第i个三维点的共视次数
			////主向量模长与副向量模长的比值>30
			//if ( (sqrt(dW1)/sqrt(dW2)>30) || (err1>err2)&&(m_archor[3*i]==2) )//主锚点残差>副锚点残差 且 共视次数为2
			//{
			//	nCount++;
			//	//主副锚点调换
			//	m_archor[i*3+1] = nN;
...
void CSBA::sba_readCameraPoseration(char *fname, double* ical )
{
	FILE *fp;
	int  ch=EOF;
	...
	{
		fprintf(stderr, "SparseBA: Cannot open calbration file %s, exiting\n", fname);
		return;
	}
	...
	if ( num != 9 )
	{
		fprintf(stderr, "SparseBA error: Format of Calibration file is wrong");
		return;
	}
...
void CSBA::sba_updateKR( double *KR, double *KdA, double *KdB, double *KdG, double *K, double *p )
{
	if ( !m_bFocal )
	{
		int i = 0;
...
//By Zuo
void CSBA::sba_constructP(double* P, double* K, double* p)
{
	if (!m_bFocal)
	{
		int i = 0;
...
#pragma optimize("", off) // 关闭优化
void CSBA::sba_jacobian(double *p, int* archor,struct sba_crsm *Uidxij, double*e,double* U,double* ea, double* V, double* eb, double *W, int n, int m, int numprojs, int *nphoto, int *nfeature )
{
	int nM, nN;	  
	...
	for ( i = 0; i < m_n3Dpts; i++ )
	{
		...
		for ( j = 0; j < numfea; j++ )
		{
			...
			sba_jacobianEachPts( m_KR, m_KdA, m_KdB, m_KdG, pa, ppt, nM, nN, nP, pAM, pAA, pPA, pPB );
...
void CSBA::sba_jacobian_RobustKernel( double *p, int* archor,struct sba_crsm *Uidxij, double*e,double* U,double* ea, double* V, double* eb, double *W, int n, int m, int numprojs, int *nphoto, int *nfeature )
{
	int nM, nN;	  
	register int i, j, ii, jj, k;
	int cnp, pnp, mnp;
...
double CSBA::nrmL2xmy(double *const e, const double *const x, const double *const y, const int n)
{
...
void CSBA::sba_cost(double* p, double* hx, int* archor)
{
	register int i;
	int cnp, pnp, mnp;
	...
	for (i = 0; i < m_n2Dprojs; i++)
	{
		...
		//KR矩阵，外参指针，三维点指针，主锚点，副锚点，当前锚点，重投影uv
		sba_reprojectEachPts(m_KR, ppa, ppt, nM, nN, nP, pmeas);
		//printf("%f %f\n", pmeas[0], pmeas[1]);
	}
}

void CSBA::readNpointsAndNprojectionsFromProj(FILE *fp, int &n3Dpts, int &nprojs)
{
	int nfirst, lineno, npts, nframes, ch, n;
	nprojs = 0;
	n3Dpts = 0;
...
void CSBA::readPointProjections(FILE *fp,double *imgpts, int *photo,int* imgptsSum, int n3Dpts, int n2Dprojs )
{
...
void CSBA::readImagePts( const char* szProj, double **imgpts, int **photo,int** imgptsSum, int &n3Dpts, int &n2Dprojs )
{
	FILE  *fpp;
...
/*初始化主锚点*/
bool CSBA::sba_initializeMainArchor( 
...
/*初始化辅锚点*/
bool CSBA::sba_initializeAssoArchor( 
	double* imgpts,
	int* photo,
	double* camera,
	double* K,
...
/*初始化其他锚点*/
bool CSBA::sba_initializeOtheArchors( 
	double* imgpts,
	int* photo,
	double* camera,
	double* K,
...
bool CSBA::sba_initializeOtheArchors_Mindw(
	double* imgpts,
	int* photo,
	double* camera,
	double* K,
...
//--------------------------------------------------------
int CSBA::sba_angle2xytGN( double *p )
{
	static int i, j;
	double* pAngle;
	double xj[3], xk[3];
...
void CSBA::sba_saveXYZ( const char* camera, const char* sz3Dpt, double *p,bool gn  )
{
	static int i = 0;
	double dx, dy, dz;
	FILE *fp = NULL, *fpc = NULL;
...
int	CSBA::sba_angle2xytLM( double *p )
{
	static int i, j;
	double* pAngle;
	double xj[3], xk[3];
...
bool CSBA::sba_parseArgs( int argc, char* argv[] )
{
	int i;
	string param;
	bool bSuccess, bRKF;
	...
	for (i = 1; i < argc; i++) 
	{
		...
		if ( strcmp( name.c_str(), "robustKernel") == 0 )
		{
			...
			if ( !bRKF )
			{
				printf( "SparseBA: Must input right robust kernel function!\n" );
				return false;
			}
		}
...
void CSBA::sba_printHelp()
{
	printf( "Sparse Bundle Adjustment General Options\n" );
	printf( "\n" );
